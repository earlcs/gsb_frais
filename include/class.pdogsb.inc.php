...
class PdoGsb {
    ...
    //retourne l'id, nom et prénom des visiteurs
    public function getLesVisiteurs() {
        $req = "select distinct id, nom, prenom from visiteur order by nom";
        $rs = PdoGsb::$monPdo->query($req);
        $ligne = $rs->fetchAll();
        return $ligne;
    }
    ...
    public function majEtatFicheFrais2($idVisiteur, $mois, $etat, $nbJustificatifs) {
        $req = "update fichefrais set idEtat = '$etat', nbJustificatifs='$nbJustificatifs',  dateModif = now() 
		where fichefrais.idVisiteur ='$idVisiteur' and fichefrais.mois = '$mois'";
        PdoGsb::$monPdo->exec($req);
    }
    ...
    //Modifie l'état d'une ligne de frais hors forfait
    //en ligne de frais hors forfait "REFUSE"
    ...
    /*public function deleteFraishorsForfait($libelle){
	$req="select libelle from lignefraishorsforfait where libelle like '%REFUSE%';";
	$ligneResultat = PdoGsb::$monPdo->query($req);
        $fetch = $ligneResultat->fetch();
        return $fetch;
    }*/
    public function deleteFraishorsForfait(){
	$req="delete from lignefraishorsforfait where libelle like '%REFUSE%';";
	$ligneResultat = PdoGsb::$monPdo->query($req);
        $fetch = $ligneResultat->fetch();
        return $fetch;
    }
